...
#
#   # Tier1+ accounts should say so; Tier0 pacing (20s/request) is very slow
#   KIMI_TIER=1 bash zkc-kimi-k3-verify-v24.sh
#
#   # Focused reruns remain available
#   ZKC_LAYERS=codex bash zkc-kimi-k3-verify-v24.sh
#   ZKC_LAYERS=anthropic bash zkc-kimi-k3-verify-v24.sh
#
#   # Host-side Codex against CC Switch (NOT in-Sprite; desktop GUI required)
#   ZKC_LAYERS=codex KIMI_CODEX_ROUTER=cc-switch bash zkc-kimi-k3-verify-v24.sh
#
...
#   P6-P8 web      Kimi Formula web-search: execution, synthesis, source verification
#
...
# Documented Moonshot limits -- https://platform.kimi.ai/docs/pricing/limits
#   Tier0  $1     concurrency 1    RPM 3      TPM 500k  TPD 1.5M
#   Tier1  $10    concurrency 50   RPM 200    TPM 2M    TPD unlimited
#   Tier2  $20    concurrency 100  RPM 500    TPM 3M    TPD unlimited
#   Tier3  $100   concurrency 200  RPM 5000   TPM 3M    TPD unlimited
# Tier0's RPM 3 requires >=20000 ms between request starts. Revisions before v24
# assumed 20 RPM and paced at 3200 ms, overrunning the real ceiling by ~6x; that
# is what blocked the long-horizon Claude Code probes (L7/L8/L9). Raw HTTP
# retries are bounded; ordinary rate limits require Retry-After, while engine
# overload uses a short bounded exponential backoff. TPD stops by default; an
# explicit KIMI_TPD_ACTION=wait permits one wait-until-reset attempt.
KIMI_TIER="${KIMI_TIER:-0}"
case "$KIMI_TIER" in
  0) _zkc_tier_interval=20000; _zkc_tier_conc=1  ;;
  1) _zkc_tier_interval=300;   _zkc_tier_conc=8  ;;
  2) _zkc_tier_interval=120;   _zkc_tier_conc=16 ;;
  *) _zkc_tier_interval=100;   _zkc_tier_conc=32 ;;
esac
KIMI_MIN_REQUEST_INTERVAL_MS="${KIMI_MIN_REQUEST_INTERVAL_MS:-$_zkc_tier_interval}"
...
KIMI_MAX_CONCURRENCY="${KIMI_MAX_CONCURRENCY:-$_zkc_tier_conc}"
...
# Tier0-safe budget and one bounded escalation when the first response ends
...
# cheapest identity/echo probes, while still remaining inexpensive on Tier0.
ANTHROPIC_OUTPUT_TOKENS="${KIMI_ANTHROPIC_PROBE_MAX_OUTPUT_TOKENS:-16384}"
# Official web-search Formula calls need a separate synthesis turn after tool
# execution. K3 thinks on every turn and reasoning tokens share this budget, so
# a 4096 cap is exhausted inside reasoning_content before any content is
# emitted -- which earlier revisions misreported as a model limitation.
...
CLIENT_OUTPUT_TOKENS="${KIMI_CLIENT_MAX_OUTPUT_TOKENS:-4096}"
# Codex routing mode. Codex sends Responses requests while Kimi exposes Chat
# Completions, so a converting router is mandatory. Kimi's own guide names CC
# Switch, but CC Switch is a desktop GUI application and cannot run inside a
# Sprite. codeproxy is the in-Sprite equivalent and performs the same
# Responses<->Chat-Completions conversion headlessly, so it is the default here.
# cc-switch remains available for manual host-side runs only.
CODEX_ROUTER="${KIMI_CODEX_ROUTER:-codeproxy}"
CC_SWITCH_BASE_URL="${KIMI_CC_SWITCH_BASE_URL:-http://127.0.0.1:15721/v1}"
...
# Cheap Codex capability probes use low reasoning; science/reasoning probes use
# a separate effort so routine checks do not waste Tier0 tokens.
CODEX_SIMPLE_REASONING_EFFORT="${KIMI_CODEX_SIMPLE_REASONING_EFFORT:-low}"
CODEX_COMPLEX_REASONING_EFFORT="${KIMI_CODEX_COMPLEX_REASONING_EFFORT:-high}"
REQUEST_TIMEOUT="${KIMI_REQUEST_TIMEOUT:-300}"
CODEX_STANDALONE_WEB_SEARCH="${CODEX_STANDALONE_WEB_SEARCH:-1}"
# Moonshot documents web search as "being updated and not recommended for use
# in the near term", so P6-P8 and L7 measure a rebuild schedule rather than K3.
# Opt in explicitly with OPEN_WEB_TEST=1 to track its recovery.
...
# Tested package versions. Remote bridge/Claude dependencies are installed
# inside the selected Sprite. CC Switch mode additionally uses a host Codex CLI;
...
if [[ "$CODEX_ROUTER" == cc-switch ]]; then
  echo "warning: KIMI_CODEX_ROUTER=cc-switch runs Codex on the HOST, not in the Sprite." >&2
  echo "         CC Switch is a desktop application and cannot run inside the container." >&2
  echo "         Use KIMI_CODEX_ROUTER=codeproxy for in-Sprite verification." >&2
fi
[[ "$CC_SWITCH_MODEL" == "$MODEL" ]] || { echo "error: KIMI_CC_SWITCH_MODEL must exactly match KIMI_MODEL ($MODEL)" >&2; exit 2; }
[[ "$CC_SWITCH_BASE_URL" =~ ^http://(127\.0\.0\.1|localhost):[0-9]{1,5}(/v1)?/?$ ]] || { echo "error: KIMI_CC_SWITCH_BASE_URL must be a localhost HTTP route such as http://127.0.0.1:15721/v1" >&2; exit 2; }
...
[[ "$CODEX_SIMPLE_REASONING_EFFORT" =~ ^(minimal|low|medium|high|xhigh)$ ]] || { echo "error: KIMI_CODEX_SIMPLE_REASONING_EFFORT must be minimal, low, medium, high, or xhigh" >&2; exit 2; }
[[ "$CODEX_COMPLEX_REASONING_EFFORT" =~ ^(minimal|low|medium|high|xhigh)$ ]] || { echo "error: KIMI_CODEX_COMPLEX_REASONING_EFFORT must be minimal, low, medium, high, or xhigh" >&2; exit 2; }
...
if (( KIMI_TIER == 0 && KIMI_MIN_REQUEST_INTERVAL_MS < 20000 )); then
  echo "warning: ${KIMI_MIN_REQUEST_INTERVAL_MS}ms between requests exceeds the documented Tier0 RPM of 3; expect 429s" >&2
fi
...
(( KIMI_MAX_CONCURRENCY >= 1 && KIMI_MAX_CONCURRENCY <= 64 )) || { echo "error: KIMI_MAX_CONCURRENCY must be 1..64" >&2; exit 2; }
if (( KIMI_TIER == 0 && KIMI_MAX_CONCURRENCY > 1 )); then
  echo "error: documented Tier0 concurrency is 1; raise KIMI_TIER or set KIMI_MAX_CONCURRENCY=1" >&2; exit 2
fi
...
for f in keepawake.pid bridge.pid; do
  [ -f "$root/$f" ] || continue
...
echo "  verifier output cap=$MAX_OUTPUT_TOKENS (routine default)"
...
if (( CODEX_USES_CC_SWITCH )); then
  echo "  codex_router=$CODEX_ROUTER  cc_switch_route=$CC_SWITCH_BASE_URL  mapped_model=$CC_SWITCH_MODEL"
else
  echo "  codex_router=$CODEX_ROUTER  in_sprite_bridge=http://127.0.0.1:$PORT/v1  mapped_model=$MODEL"
fi
...
echo "  tier=$KIMI_TIER guardrail max_concurrency=$KIMI_MAX_CONCURRENCY min_interval_ms=$KIMI_MIN_REQUEST_INTERVAL_MS"
echo "  raw_anthropic_api_model=$ANTHROPIC_API_MODEL  claude_code_selector=$CLAUDE_MODEL"
if (( CODEX_USES_CC_SWITCH )); then
  note "HOST MODE: Codex runs on this host against CC Switch, NOT inside the Sprite"
else
  note "Codex runs inside the Sprite against the in-Sprite codeproxy Responses bridge"
fi
if (( L_PROTOCOL && L_ANTHROPIC && L_CODEX && L_CLAUDE )); then
  note "full matrix enabled: protocol, Anthropic, Codex, Claude Code, web, science, and long-context probes will run"
fi
...
layer_probe_names(){
  case "$1" in
    ...
    codex) printf '%s\n' "C0-config C1-shell C2-chained C3-parallel C4-create C5-edit C6-recover C7-config C7-catalog C7-context C8-nativeweb C9-science C10-reasoning";;
    claude) printf '%s\n' "L1-shell L2-chained L3-parallel L4-create L5-edit L6-recover L7-websearch L8-science L9-reasoning";;
    *) return 2;;
  esac
}
...
model_reasoning_effort = "${Z_DEFAULT_EFFORT:-high}"
...
step "1. prepare"
if (( NEED_SPRITE )); then
  run_remote "$S_PREP" "Z_RUN_ID=$RUN_ID,Z_FORCE_INSTALL=$ZKC_FORCE_INSTALL,Z_NEED_CODEPROXY=$REMOTE_NEED_CODEPROXY,Z_NEED_CODEX=$REMOTE_NEED_CODEX,Z_NEED_CLAUDE=$L_CLAUDE,Z_CODEPROXY_VERSION=$CODEPROXY_VERSION,Z_CODEX_VERSION=$CODEX_VERSION,Z_CLAUDE_VERSION=$CLAUDE_CODE_VERSION,Z_DEFAULT_EFFORT=$CODEX_COMPLEX_REASONING_EFFORT"
  PREP_RC=$?
  if (( PREP_RC != 0 )); then
...
DEFAULT_EFFORT="${B_DEFAULT_EFFORT:-high}"
...
python3 - "$CFG" "$PORT" "$MODEL" "$TIMEOUT" 3<<<"$K" <<'PY'
import json, os, sys
cfg,port,model,timeout=sys.argv[1:5]
key=os.read(3,65536).decode().rstrip("\n")
data={"version":"1.0","port":int(port),"currentUpstream":"kimi",
      "timeoutMs":int(timeout)*1000,
      "upstreams":{"kimi":{"baseUrl":"https://api.moonshot.ai/v1",
...
for i in $(seq 1 60); do
  ...
  [ "$i" = 60 ] && {
    echo '       NO BIND. redacted log tail:'
    tail -20 "$ROOT/bridge.log" | sed -E 's/sk-[A-Za-z0-9._-]{8,}/sk-REDACTED/g; s/^/         /'
    exit 74
  }
  sleep 2
done
...
python3 - "$CODEX_HOME/config.toml" "$CODEX_HOME/kimi.config.toml" "$CODEX_HOME/models.json" "$PORT" "$MODEL" "$CONTEXT" "$MAXOUT" "$WEB_NATIVE" "$REASONING_SUMMARIES" "$DEFAULT_EFFORT" <<'PY'
import json, os, sys
cfg,profile,catalog,port,model,context,maxout,web_native,reasoning_summaries,default_effort=sys.argv[1:11]
entry = {
    "slug": model,
...
with open(profile,"w") as f:
    ...
    f.write('model_reasoning_effort = "%s"\n' % default_effort)
...
echo "       bridge endpoint=http://127.0.0.1:$PORT/v1"
...
step "3. bridge"
if (( NEED_BRIDGE )); then
  if send_secret "$S_BRIDGE" "Z_RUN_ID=$RUN_ID,Z_TASK=$TASK_NAME,B_PORT=$PORT,B_PORT_SPAN=$BRIDGE_PORT_SPAN,B_PORT_STRICT=$BRIDGE_PORT_STRICT,B_MODEL=$MODEL,B_CONTEXT=$CONTEXT_WINDOW,B_MAXOUT=$CLIENT_OUTPUT_TOKENS,B_REASONING_SUMMARIES=$CODEX_REASONING_SUMMARIES,B_MODEL_DEFAULT=$MODEL_DEFAULT_MAX_OUTPUT_TOKENS,B_WEB_NATIVE=$CODEX_STANDALONE_WEB_SEARCH,B_TIMEOUT=$REQUEST_TIMEOUT,B_KEEP_BRIDGE=$KEEP_BRIDGE,B_DEFAULT_EFFORT=$CODEX_COMPLEX_REASONING_EFFORT,KEEP_MAX=$KEEPAWAKE_MAX"; then
    ACTUAL_PORT=$(run_remote 'cat "$HOME/.cache/zkc-kimi-k3/$Z_RUN_ID/bridge.port"' "Z_RUN_ID=$RUN_ID" 2>/dev/null | tr -d '\r\n')
    is_uint "$ACTUAL_PORT" && (( ACTUAL_PORT >= 1 && ACTUAL_PORT <= 65535 )) || {
      bad "bridge started but did not report a valid port"
      exit 1
    }
    PORT="$ACTUAL_PORT"
  ...
  if (( CODEX_USES_CC_SWITCH )); then
    note "not started; Codex will use host CC Switch Local Routing at $CC_SWITCH_BASE_URL"
  else
    note "not started; selected layers do not require the Responses bridge"
  fi
fi
...
set -uo pipefail
IFS= read -r K || true
PORT="${B_PORT:?}"; MODEL="${B_MODEL:?}"; TIMEOUT="${B_TIMEOUT:-300}"
...
python3 -u - "$PORT" "$MODEL" "$TIMEOUT" "$WEB_TEST" "$WEB_AGE" "$PROBE_MAX" "$REASONING_MAX" "$REASONING_RETRY_MAX" "$HTTP_LOG_LEVEL" "$HTTP_BODY_MAX" "$MIN_INTERVAL_MS" "$RATE_RETRIES" "$RETRY_MAX" "$ENGINE_RETRIES" "$ENGINE_BASE" "$WEB_MAX" "$TPD_ACTION" "$TPD_WAIT_MAX" "$TPD_GRACE" "$RESET_TZ" 3<<<"$K" <<'PYEOF'
import copy
...
PORT, MODEL, TIMEOUT, WEB_TEST, WEB_AGE, PROBE_MAX, REASONING_MAX, REASONING_RETRY_MAX, HTTP_LOG_LEVEL, HTTP_BODY_MAX, MIN_INTERVAL_MS, RATE_RETRIES, RETRY_MAX, ENGINE_RETRIES, ENGINE_BASE, WEB_MAX, TPD_ACTION, TPD_WAIT_MAX, TPD_GRACE, RESET_TZ = sys.argv[1:21]
...
if not WEB_TEST:
    probe("P6-webtool", "SKIP", "OPEN_WEB_TEST=0")
    probe("P7-websynth", "SKIP", "OPEN_WEB_TEST=0")
    probe("P8-webverify", "SKIP", "OPEN_WEB_TEST=0")
else:
    ...
    formula_base="https://api.moonshot.ai/v1/formulas/"+formula_uri
    ...
    if tools_http==200 and formula_tools:
        ...
        if web_http == 200 and calls > 0 and fibers_ok == calls and not final_text:
            ...
            if web_http == 200:
                try:
                    wd=json.loads(wr)
                    final_choice=wd["choices"][0]
                    final_text=(final_choice["message"].get("content") or "")
                    final_finish=final_choice.get("finish_reason") or ""
                except Exception:
                    final_text=""; final_finish=""
    ...
    elif p6_ready and final_finish == "length":
        probe("P6-webtool","TRUNCATED",
              "harness cap reached: max_completion_tokens=%d was exhausted inside "
              "reasoning_content before any content was emitted; raise "
              "KIMI_WEB_PROBE_MAX_OUTPUT_TOKENS"%WEB_MAX)
    elif p6_ready:
...
IFS= read -r K || true
M="${A_MODEL:?}"; TIMEOUT="${A_TIMEOUT:-300}"; MAXTOK="${A_MAXTOK:-1024}"
...
python3 -u - "$M" "$TIMEOUT" "$MAXTOK" "$HTTP_LOG_LEVEL" "$HTTP_BODY_MAX" "$MIN_INTERVAL_MS" "$RATE_RETRIES" "$RETRY_MAX" "$ENGINE_RETRIES" "$ENGINE_BASE" "$TPD_ACTION" "$TPD_WAIT_MAX" "$TPD_GRACE" "$RESET_TZ" 3<<<"$K" <<'PYEOF'
...
M, TIMEOUT, MAXTOK, HTTP_LOG_LEVEL, HTTP_BODY_MAX = sys.argv[1], int(sys.argv[2]), int(sys.argv[3]), sys.argv[4], int(sys.argv[5])
MIN_INTERVAL = int(sys.argv[6]) / 1000.0
RATE_RETRIES, RETRY_MAX = int(sys.argv[7]), int(sys.argv[8])
ENGINE_RETRIES, ENGINE_BASE = int(sys.argv[9]), int(sys.argv[10])
TPD_ACTION, TPD_WAIT_MAX, TPD_GRACE, RESET_TZ = sys.argv[11], int(sys.argv[12]), int(sys.argv[13]), sys.argv[14]
LAST_REQUEST_STARTED = 0.0
...
if [ "${B_API_BLOCKED:-0}" = 1 ]; then
  for n in C0-config C1-shell C2-chained C3-parallel C4-create C5-edit C6-recover C7-config C7-catalog C7-context C8-nativeweb C9-science C10-reasoning; do
    printf '  %-14s %-7s %s
PROBE|%s|BLOCKED|%s
...
SIMPLE_EFFORT="${B_SIMPLE_EFFORT:-low}"; COMPLEX_EFFORT="${B_COMPLEX_EFFORT:-high}"
W="$ZKC_WORK"; cd "$W"
p(){ printf '  %-14s %-5s %s\n' "$1" "$2" "${3:-}"; echo "PROBE|$1|$2|${3:-}"; }
CX=("$CODEX_BIN" exec --profile kimi --ephemeral --dangerously-bypass-approvals-and-sandbox
    -c web_search=disabled -c model_reasoning_effort="$SIMPLE_EFFORT")
...
# Codex has no `config get` subcommand (`-c/--config` is a write-side override
# only), so earlier revisions silently produced empty strings here and the
# model/context assertions degraded into no-ops. Read the generated TOML.
codex_cfg_get(){
  python3 - "$CODEX_HOME/kimi.config.toml" "$CODEX_HOME/config.toml" "$1" <<'PYCFG'
import sys
try:
    import tomllib
...
o=$(run "Reply with only the word READY.")
C0_MODEL=$(codex_cfg_get model | tr -d ' \r\n' || true)
if grep -qiE 'sign in with chatgpt|codex login|requires.*openai|authentication.*openai|legacy profile|model catalog.*(failed|error)' <<<"$o"; then
  p C0-config FAIL "profile/auth/catalog preflight failed"
  tail -10 <<<"$o" | sed 's/^/         /'
  for n in C1-shell C2-chained C3-parallel C4-create C5-edit C6-recover C7-config C7-catalog C7-context C8-nativeweb C9-science C10-reasoning; do
    p "$n" BLOCKED "not attempted after C0-config failure"
  done
  exit 0
elif grep -q READY <<<"$o" && [ "$C0_MODEL" = "$MODEL" ]; then
  p C0-config PASS "custom provider loaded without OpenAI authentication; model=$C0_MODEL"
else
  p C0-config FAIL "preflight returned no READY or wrong model=${C0_MODEL:-unset}"
  tail -10 <<<"$o" | sed 's/^/         /'
  for n in C1-shell C2-chained C3-parallel C4-create C5-edit C6-recover C7-config C7-catalog C7-context C8-nativeweb C9-science C10-reasoning; do
    p "$n" BLOCKED "not attempted after C0-config failure"
  done
...
if cmp -s edit.txt "$ZKC_TMP/c5.expected"; then
...
EFF=$(codex_cfg_get model_context_window | tr -d ' \r\n' || true)
EOUT=$(codex_cfg_get model_max_output_tokens | tr -d ' \r\n' || true)
...
PYCTX
  lo=$(cat "$ZKC_TMP/zkc-long-prompt.txt" | timeout "$TIMEOUT" "$CODEX_BIN" exec --profile kimi --ephemeral \
       --dangerously-bypass-approvals-and-sandbox -c web_search=disabled -c model_reasoning_effort="$SIMPLE_EFFORT" - 2>&1)
  if grep -q "$SENT" <<<"$lo"; then
...
if [ "$WEB_NATIVE" != 1 ]; then
  ...
  o=$(timeout "$TIMEOUT" "$CODEX_BIN" exec --profile kimi --ephemeral --dangerously-bypass-approvals-and-sandbox \
        ...
        "Search the live web for a scientific announcement from the last 7 days. Return title, date, and URL." 2>&1)
  WEB_EVENT=$(printf '%s\n' "$o" | python3 -c '
import json,sys
...
if [ "$SCIENCE" = 1 ]; then
  ...
  so=$(cat science_prompt.txt | timeout "$TIMEOUT" "$CODEX_BIN" exec --profile kimi --ephemeral \
      ...
      -c model_reasoning_effort="$COMPLEX_EFFORT" --json - 2>&1)
  SCI_OK=0
...
resolve_host_codex_bin(){
  ...
  # Report which of the three conditions actually stopped us. Collapsing them
  # into one string previously made the failure unreadable from the transcript.
  if [ "$AUTO_INSTALL_HOST_CODEX" != 1 ]; then
    HOST_CODEX_FAIL_REASON="codex not on PATH and KIMI_AUTO_INSTALL_HOST_CODEX=0"; return 1
  fi
  if ! command -v npm >/dev/null 2>&1; then
    HOST_CODEX_FAIL_REASON="codex not on PATH and npm is unavailable to install it"; return 1
  fi
  if ! mkdir -p "$HOST_CODEX_PREFIX" 2>/dev/null; then
    HOST_CODEX_FAIL_REASON="cannot create install prefix $HOST_CODEX_PREFIX"; return 1
  fi
  printf '       host Codex CLI not found; installing @openai/codex@%s under %s ...\n'     "$CODEX_VERSION" "$HOST_CODEX_PREFIX" >&2
  if ! npm install --prefix "$HOST_CODEX_PREFIX" --no-audit --no-fund --loglevel=error     "@openai/codex@$CODEX_VERSION" >"$HOST_CODEX_PREFIX/npm-install.log" 2>&1; then
    HOST_CODEX_FAIL_REASON="npm install failed; see $HOST_CODEX_PREFIX/npm-install.log"; return 1
  fi
  candidate="$HOST_CODEX_PREFIX/node_modules/.bin/codex"
  [ -x "$candidate" ] || { HOST_CODEX_FAIL_REASON="npm install produced no executable"; return 1; }
  printf '%s\n' "$candidate"
}
run_codex_cc_switch_host(){
  ...
  codex_bin=$(resolve_host_codex_bin) || {
    local why="${HOST_CODEX_FAIL_REASON:-host Codex CLI unavailable}"
    printf '  %-14s %-7s %s\nPROBE|C0-config|ENVERR|%s\n' C0-config ENVERR "$why" "$why"
    printf '       host mode requires a host Codex CLI; for in-Sprite verification use KIMI_CODEX_ROUTER=codeproxy\n' >&2
    for n in C1-shell C2-chained C3-parallel C4-create C5-edit C6-recover C7-config C7-catalog C7-context C8-nativeweb C9-science C10-reasoning; do
      printf '  %-14s %-7s %s\nPROBE|%s|BLOCKED|%s\n' "$n" BLOCKED 'not attempted after C0-config environment error' "$n" 'not attempted after C0-config environment error'
    done
    return 0
  }
  # Codex exposes no `config get` subcommand; `-c/--config` is a write-side
  # override only. Read the config.toml that CC Switch generates instead.
  local codex_cfg="${CODEX_HOME:-$HOME/.codex}/config.toml"
  if [ ! -r "$codex_cfg" ]; then
    printf '  %-14s %-7s %s\nPROBE|C0-config|ENVERR|%s\n' C0-config ENVERR \
      "no readable Codex config at $codex_cfg" "no readable Codex config at $codex_cfg"
    for n in C1-shell C2-chained C3-parallel C4-create C5-edit C6-recover C7-config C7-catalog C7-context C8-nativeweb C9-science C10-reasoning; do
      printf '  %-14s %-7s %s\nPROBE|%s|BLOCKED|%s\n' "$n" BLOCKED 'Codex configuration not found; enable CC Switch Codex takeover first' "$n" 'Codex configuration not found; enable CC Switch Codex takeover first'
    done
    return 0
  fi
  IFS=$'\t' read -r active_model active_provider active_base active_context catalog < <(
    python3 - "$codex_cfg" <<'PYCFG'
import sys
try:
    import tomllib
except ModuleNotFoundError:
...
base     = str((cfg.get("model_providers", {}).get(provider) or {}).get("base_url") or "")
ctx      = str(cfg.get("model_context_window") or "")
catalog  = str(cfg.get("model_catalog_json") or "")
print("\t".join((model, provider, base, ctx, catalog)))
PYCFG
  )

  # CC Switch's docs warn its local-routing port is not fixed; prefer the value
  # Codex actually holds and fall back to the configured default.
  route="${active_base:-$CC_SWITCH_BASE_URL}"
...
PYSOCK
  then
    printf '  %-14s %-5s %s\nPROBE|C0-config|FAIL|%s\n' C0-config FAIL "CC Switch Local Routing is not listening at $route" "CC Switch Local Routing is not listening at $route"
    for n in C1-shell C2-chained C3-parallel C4-create C5-edit C6-recover C7-config C7-catalog C7-context C8-nativeweb C9-science C10-reasoning; do
      printf '  %-14s %-7s %s\nPROBE|%s|BLOCKED|%s\n' "$n" BLOCKED 'enable CC Switch Routing Master Switch and Codex takeover first' "$n" 'enable CC Switch Routing Master Switch and Codex takeover first'
    done
    return 0
  fi
  ...
  # route is now derived from active_base, so compare against the expected
  # localhost shape instead: a non-loopback provider means takeover is off.
  if [ -z "$active_base" ]; then
    p_host C0-config FAIL "active provider $active_provider declares no base_url"
    block_rest "not attempted until Codex takeover points at the CC Switch local route"
    return 0
  fi
  if [[ ! "$active_base" =~ ^http://(127\.0\.0\.1|localhost): ]]; then
    p_host C0-config FAIL "active provider route=$active_base is not a CC Switch local route"
    block_rest "not attempted until Codex takeover points at the CC Switch local route"
    return 0
  fi
  [ "${active_base%/}" = "${CC_SWITCH_BASE_URL%/}" ] || \
    printf '       note: CC Switch route is %s, not the configured default %s\n' "$active_base" "$CC_SWITCH_BASE_URL" >&2
...
if (( L_CODEX )); then
  ...
  printf '%s\n' "$CODEX_OUT" | grep -qiE 'TPD|daily token|exceeded_current_quota|current:[[:space:]]*[0-9]+.*limit:' && API_BLOCKED=1
else
...
if [ "${A_API_BLOCKED:-0}" = 1 ]; then
  for n in L1-shell L2-chained L3-parallel L4-create L5-edit L6-recover L7-websearch L8-science L9-reasoning; do
    want "$n" || continue
    printf '  %-14s %-7s %s
PROBE|%s|BLOCKED|%s
' "$n" BLOCKED 'not attempted after a Moonshot rate-limit response' "$n" 'not attempted after a Moonshot rate-limit response'
...
RUN_ID="${Z_RUN_ID:?}"; ROOT="$HOME/.cache/zkc-kimi-k3/$RUN_ID"
. "$ROOT/runtime.env"
CMODEL="${A_MODEL:?}"; CONTEXT="${A_CONTEXT:-1048576}"; TIMEOUT="${A_TIMEOUT:-300}"
...
# Required by Moonshot's documented Claude Code configuration. Tool search
# became a client default and assumes server-side support that the /anthropic
# compatibility shim does not necessarily implement.
export ENABLE_TOOL_SEARCH=false
# Note: Moonshot's guide targets kimi-k2.7-code on /anthropic and sets
# CLAUDE_CODE_AUTO_COMPACT_WINDOW=262144. This verifier deliberately runs
# kimi-k3 at the full context window; A1-A4 confirm the endpoint accepts it.
...
claude_capture(){
  ...
  if [ "$CLAUDE_CONFIG_BLOCKED" = 1 ]; then
    return 76
  fi
  while :; do
    CLAUDE_LAST=$(claude_call "$tools" "$@" 2>&1)
    rc=$?
    if grep -qiE 'engine_overloaded_error|engine is currently overloaded' <<<"$CLAUDE_LAST" &&        [ "$attempt" -lt "$ENGINE_RETRIES" ]; then
      delay=$(( ENGINE_BASE * (1 << attempt) ))
  ...
  if grep -qiE 'TPD|daily token|exceeded_current_quota|current:[[:space:]]*[0-9]+.*limit:' <<<"$CLAUDE_LAST"; then
    CLAUDE_QUOTA_BLOCKED=1
    CLAUDE_QUOTA_DETAIL="Moonshot TPD/quota limit reached; remaining probes in this Claude session not attempted"
    return 75
  fi
  # Match transport error envelopes only. Earlier revisions grepped the whole
  # transcript, so the model writing "429" or a fetched page containing "rate
  # limit" set a sticky quota flag that blocked every later probe in the group.
  if grep -qiE 'API Error: *429|"status" *: *429|rate_limit_error|engine_overloaded_error|engine is currently overloaded' <<<"$CLAUDE_LAST"; then
    CLAUDE_QUOTA_BLOCKED=1
    CLAUDE_QUOTA_DETAIL="Moonshot transient capacity/rate response remained after bounded retries; this probe is blocked"
    return 75
  fi
...
quota_detail(){ printf '%s' "${CLAUDE_QUOTA_DETAIL:-Moonshot API capacity/rate response; probe not completed}"; }
...
# Classify a Claude Code stream-json run from its EVENTS, never its prose. The
# transcript holds model output, tool calls and fetched page text, so a text
# grep cannot distinguish a transport 429 from the model writing "429".
# Echoes exactly one of: ok | ratelimited | apierror | timeout | empty
claude_stream_status(){
  local raw="$1" rc="$2"
  [ "$rc" = 124 ] && { printf 'timeout'; return 0; }
  python3 - "$raw" <<'PYST'
import json, sys
status = "empty"
try:
    ...
    print(status); raise SystemExit(0)
for line in fh:
    line = line.strip()
    ...
    if ev.get("type") == "result":
        if ev.get("is_error"):
            sub = str(ev.get("subtype") or ev.get("error_type") or "").lower()
            status = "ratelimited" if ("rate" in sub or "429" in sub or "quota" in sub) else "apierror"
        else:
            status = "ok"
    err = ev.get("error")
    if isinstance(err, dict):
        code = str(err.get("status") or err.get("code") or "")
        etype = str(err.get("type") or "").lower()
        if code == "429" or "rate_limit" in etype or "overloaded" in etype:
            status = "ratelimited"
...
Reply with only what it printed."; then
  ...
  rc=$?; [ "$rc" = 75 ] && p L1-shell BLOCKED "$(quota_detail)" || { [ "$rc" = 76 ] && p L1-shell BLOCKED "$(config_detail)" || { p L1-shell FAIL "claude exited $rc"; show_tail "$CLAUDE_LAST"; }; }
fi
...
Reply with only the final number."; then
  ...
  rc=$?; [ "$rc" = 75 ] && p L2-chained BLOCKED "$(quota_detail)" || { [ "$rc" = 76 ] && p L2-chained BLOCKED "$(config_detail)" || { p L2-chained FAIL "claude exited $rc"; show_tail "$CLAUDE_LAST"; }; }
fi
...
Reply with all three outputs."; then
  ...
  rc=$?; [ "$rc" = 75 ] && p L3-parallel BLOCKED "$(quota_detail)" || { [ "$rc" = 76 ] && p L3-parallel BLOCKED "$(config_detail)" || { p L3-parallel FAIL "claude exited $rc"; show_tail "$CLAUDE_LAST"; }; }
fi
...
PYDIAG
  ...
  rc=$?; [ "$rc" = 75 ] && p L4-create BLOCKED "$(quota_detail)" || { [ "$rc" = 76 ] && p L4-create BLOCKED "$(config_detail)" || { p L4-create FAIL "claude exited $rc"; show_tail "$CLAUDE_LAST"; }; }
fi
...
if claude_capture "Read,Edit" "In edit.txt replace the line OLDVALUE with NEW$N. Change nothing else."; then
  ...
  rc=$?; [ "$rc" = 75 ] && p L5-edit BLOCKED "$(quota_detail)" || { [ "$rc" = 76 ] && p L5-edit BLOCKED "$(config_detail)" || { p L5-edit FAIL "claude exited $rc"; show_tail "$CLAUDE_LAST"; }; }
fi
...
Reply with only the second output."; then
  ...
  rc=$?; [ "$rc" = 75 ] && p L6-recover BLOCKED "$(quota_detail)" || { [ "$rc" = 76 ] && p L6-recover BLOCKED "$(config_detail)" || { p L6-recover FAIL "claude exited $rc"; show_tail "$CLAUDE_LAST"; }; }
fi
...
TXT
  ...
  ANTHROPIC_AUTH_TOKEN="$K" timeout "$TIMEOUT" "$CLAUDE_BIN" -p "$L7PROMPT" \
    ...
    --tools "WebSearch" --output-format stream-json --verbose --max-turns 10 \
    >"$L7RAW" 2>&1
  L7RC=$?
  L7STATUS=$(claude_stream_status "$L7RAW" "$L7RC")
  if [ "$L7STATUS" = ratelimited ]; then
    CLAUDE_QUOTA_BLOCKED=1
    p L7-websearch BLOCKED "$(quota_detail)"
  elif [ "$L7STATUS" = timeout ]; then
    p L7-websearch TIMEOUT "no result event within ${TIMEOUT}s"
  elif [ "$L7STATUS" = apierror ]; then
    p L7-websearch FAIL "API error event in stream (claude exited $L7RC)"
    tail -10 "$L7RAW" | sed 's/^/         /'
  else
...
PYWEB
  ...
  p L7-websearch SKIP "OPEN_WEB_TEST=0"
fi
...
if [ "$CLAUDE_QUOTA_BLOCKED" = 1 ]; then
  ...
  ANTHROPIC_AUTH_TOKEN="$K" timeout "$TIMEOUT" "$CLAUDE_BIN" -p "$L8PROMPT" \
    --dangerously-skip-permissions --strict-mcp-config --disable-slash-commands --no-session-persistence \
    --tools "Bash,Read,Write,Edit" --output-format stream-json --verbose --max-turns 20 \
    >"$L8RAW" 2>&1
  L8RC=$?
  L8STATUS=$(claude_stream_status "$L8RAW" "$L8RC")
  if [ "$L8STATUS" = ratelimited ]; then
    CLAUDE_QUOTA_BLOCKED=1
    p L8-science BLOCKED "$(quota_detail)"
    p L9-reasoning BLOCKED "$(quota_detail)"
  elif [ "$L8STATUS" = timeout ]; then
    p L8-science TIMEOUT "no result event within ${TIMEOUT}s"
    p L9-reasoning TIMEOUT "no result event within ${TIMEOUT}s"
  else
  LSCI_OK=0
...
if (( L_CLAUDE )); then
  ...
  for _claude_group in L1-shell L2-chained L3-parallel L4-create L5-edit L6-recover L7-websearch 'L8-science,L9-reasoning'; do
    _claude_part=$(send_secret "$S_CLAUDE" "Z_RUN_ID=$RUN_ID,A_ONLY=$_claude_group,A_TIMEOUT=$REQUEST_TIMEOUT,A_MODEL=$CLAUDE_MODEL,A_CONTEXT=$CONTEXT_WINDOW,A_WEB_TEST=$OPEN_WEB_TEST,A_WEB_AGE=$WEB_MAX_AGE_DAYS,A_SCIENCE=$SCIENCE_TEST,A_API_BLOCKED=$API_BLOCKED,A_MIN_INTERVAL_MS=$KIMI_MIN_REQUEST_INTERVAL_MS,A_ENGINE_RETRIES=$KIMI_ENGINE_OVERLOAD_RETRIES,A_ENGINE_BASE=$KIMI_ENGINE_OVERLOAD_BASE_SECONDS,A_RETRY_MAX=$KIMI_RETRY_MAX_SECONDS" 2>&1 | tee /dev/stderr)
    _claude_rc=$?
    CLAUDE_OUT+="${CLAUDE_OUT:+$'\n'}$_claude_part"
    if printf '%s\n' "$_claude_part" | grep -qiE 'TPD|daily token|exceeded_current_quota|current:[[:space:]]*[0-9]+.*limit:'; then
      API_BLOCKED=1
    fi
    if [ "$_claude_rc" -ne 0 ]; then CLAUDE_RC="$_claude_rc"; fi
  done
  unset _claude_group _claude_part _claude_rc
else
...
printf '%s\n' "$PROBE_ROWS" | while IFS='|' read -r _ n v d; do
  case "$v" in
    ...
    BLOCKED)   printf '  \033[1;35m%-14s %-7s\033[0m %s\n' "$n" BLOCKED "$d" ;;
    ENVERR)    printf '  \033[1;34m%-14s %-7s\033[0m %s\n' "$n" ENVERR "$d" ;;
    TRUNCATED) printf '  \033[1;33m%-14s %-7s\033[0m %s\n' "$n" TRUNCATED "$d" ;;
    TIMEOUT)   printf '  \033[1;33m%-14s %-7s\033[0m %s\n' "$n" TIMEOUT "$d" ;;
    *)         printf '  \033[1;33m%-14s %-7s\033[0m %s\n' "$n" "$v" "$d" ;;
  esac
done
...
# An environment error is a harness precondition, not a model capability
# finding, and must never be counted toward the run's failure total.
...
cf(){ cv "$1" FAIL; }
group_summary(){
  local prefix="$1" f b l s e t x extra
  f=$(cv "$prefix" FAIL); b=$(cv "$prefix" BLOCKED); l=$(cv "$prefix" LIMIT); s=$(cv "$prefix" SKIP)
  e=$(cv "$prefix" ENVERR); t=$(cv "$prefix" TRUNCATED); x=$(cv "$prefix" TIMEOUT)
  extra=""
  (( e )) && extra+=", $e environment error(s)"
  (( t )) && extra+=", $t truncated"
  (( x )) && extra+=", $x timed out"
  printf '%s failure(s), %s blocked, %s limitation(s), %s skipped%s' "$f" "$b" "$l" "$s" "$extra"
}
...
if (( L_PROTOCOL )); then SELECTED_FAILS=$((SELECTED_FAILS + BRIDGE_PROTOCOL_FAILS)); SELECTED_BLOCKEDS=$((SELECTED_BLOCKEDS + BRIDGE_PROTOCOL_BLOCKED)); SELECTED_LIMITS=$((SELECTED_LIMITS + $(cv P LIMIT))); SELECTED_SKIPS=$((SELECTED_SKIPS + $(cv P SKIP))); fi
if (( L_ANTHROPIC )); then SELECTED_FAILS=$((SELECTED_FAILS + ANTHROPIC_PROTOCOL_FAILS)); SELECTED_BLOCKEDS=$((SELECTED_BLOCKEDS + ANTHROPIC_PROTOCOL_BLOCKED)); SELECTED_LIMITS=$((SELECTED_LIMITS + $(cv A LIMIT))); SELECTED_SKIPS=$((SELECTED_SKIPS + $(cv A SKIP))); fi
if (( L_CODEX )); then SELECTED_FAILS=$((SELECTED_FAILS + CODEX_E2E_FAILS)); SELECTED_BLOCKEDS=$((SELECTED_BLOCKEDS + CODEX_E2E_BLOCKED)); SELECTED_LIMITS=$((SELECTED_LIMITS + $(cv C LIMIT))); SELECTED_SKIPS=$((SELECTED_SKIPS + $(cv C SKIP))); fi
if (( L_CLAUDE )); then SELECTED_FAILS=$((SELECTED_FAILS + CLAUDE_E2E_FAILS)); SELECTED_BLOCKEDS=$((SELECTED_BLOCKEDS + CLAUDE_E2E_BLOCKED)); SELECTED_LIMITS=$((SELECTED_LIMITS + $(cv L LIMIT))); SELECTED_SKIPS=$((SELECTED_SKIPS + $(cv L SKIP))); fi
...
if (( L_CODEX && L_CLAUDE )); then
  if [ "$CODEX_E2E_FAILS" = 0 ] && [ "$CLAUDE_E2E_FAILS" = 0 ] && \
     [ "$CODEX_E2E_BLOCKED" = 0 ] && [ "$CLAUDE_E2E_BLOCKED" = 0 ]; then
    echo; echo "  Both selected clients are operational for the tested workflows."
  else
    echo; echo "  One or both selected client paths failed or were blocked; inspect C- and L-probes."
  fi
elif (( L_CODEX )); then
  if [ "$CODEX_E2E_FAILS" = 0 ] && [ "$CODEX_E2E_BLOCKED" = 0 ]; then
    echo; echo "  The selected Codex layer completed without FAIL or BLOCKED."
  else
    echo; echo "  The selected Codex layer failed or was blocked; inspect C-probes."
  fi
elif (( L_CLAUDE )); then
  if [ "$CLAUDE_E2E_FAILS" = 0 ] && [ "$CLAUDE_E2E_BLOCKED" = 0 ]; then
    echo; echo "  The selected Claude Code layer completed without FAIL or BLOCKED."
  else
    echo; echo "  The selected Claude Code layer failed or was blocked; inspect L-probes."
  fi
else
...
set +u
if [ "${SELECTED_FAILS:-1}" = 0 ] && [ "${SELECTED_BLOCKEDS:-1}" = 0 ]; then
  cat <<TXT
  ...
  Codex through CC Switch (run Codex on this host):

    CC Switch -> Settings -> Routing:
      Routing Master Switch: On
      Routing Enabled / Codex: On

    CC Switch -> Codex -> Kimi provider:
      API request URL: https://api.moonshot.ai/v1
      Upstream format: Chat Completions (routing required)
      Default model: ${MODEL}
      Menu display name: ${MODEL}
      Actual request model: ${MODEL}
      Context window: ${CONTEXT_WINDOW}

    # Keep CC Switch routing active, restart Codex, then run:
    codex
    codex exec --ephemeral "..."

  The verifier checks the host route at ${CC_SWITCH_BASE_URL}, requires the
  active Codex model to equal ${MODEL}, and rejects a mismatched context when
  Codex exposes it. That path runs Codex on the HOST: CC Switch is a desktop
  GUI application and cannot run inside a Sprite. The default
  KIMI_CODEX_ROUTER=codeproxy runs Codex inside the Sprite against the local
  Responses bridge and performs the same protocol conversion headlessly.
  ...
  Raw protocol probes use ${MAX_OUTPUT_TOKENS} tokens and Anthropic tool probes
  use ${ANTHROPIC_OUTPUT_TOKENS}. The codeproxy compatibility profile uses a
  ${CLIENT_OUTPUT_TOKENS}-token Codex cap; CC Switch mode leaves Codex output
  policy under the active CC Switch-managed configuration. Top-level request
  starts are spaced by at least ${KIMI_MIN_REQUEST_INTERVAL_MS} ms,
  ...
  Focused reruns remain available:
    ZKC_LAYERS=claude OPEN_WEB_TEST=1 SCIENCE_TEST=1 bash $0
    ZKC_LAYERS=codex KIMI_CODEX_ROUTER=cc-switch OPEN_WEB_TEST=1 SCIENCE_TEST=1 bash $0
    ZKC_LAYERS=protocol,anthropic OPEN_WEB_TEST=1 bash $0
TXT
  if (( ! L_CODEX && ! L_CLAUDE )); then
  ...
  echo "  The selected run is incomplete: ${SELECTED_BLOCKEDS} selected probe(s) were BLOCKED."
  echo "  No selected probe returned FAIL; this is an external-capacity result, not a model failure."
  if [ -n "${TPD_GUARD_LINE:-}" ]; then
    echo
    echo "  Tier0 daily token allowance exhausted: ${TPD_CURRENT:-?} / ${TPD_LIMIT:-?} tokens"
  ...
  echo "  ${SELECTED_FAILS} selected probe(s) failed and ${SELECTED_BLOCKEDS} were blocked."
  if [ "${ENVERRS:-0}" -gt 0 ]; then
    echo "  ${ENVERRS} probe(s) reported ENVERR: the harness could not run, which says"
    echo "  nothing about the model. Fix those preconditions before reading the matrix."
  fi
